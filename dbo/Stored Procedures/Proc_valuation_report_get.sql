CREATE PROCEDURE [dbo].[Proc_valuation_report_get](@VehicleID AS VARCHAR(MAX) = NULL)
AS
    BEGIN
        SET NOCOUNT ON;
        ...
        SELECT ROW_NUMBER() OVER(PARTITION BY PV.portfolioid
               ...
               valuationmethodname
        INTO #tmpvaluation
        FROM #tmpportfoliocompany P
             JOIN [tbl_companycontact] CC ON CC.companycontactid = p.companycontactid
