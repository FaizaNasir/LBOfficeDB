CREATE PROC [dbo].[GetExcelUnderlyingInvestments]
AS
    BEGIN
        SELECT *
        FROM
        (
            SELECT i.PortfolioFundUnderlyingInvestmentsID ID, 
            (
                SELECT name
                FROM tbl_vehicle
                WHERE vehicleid =
            ...
            (
                SELECT TOP 1 *
                FROM tbl_PortfolioFundUnderlyingInvestmentsTrimester j
                WHERE i.PortfolioFundUnderlyingInvestmentsID = j.PortfolioFundUnderlyingInvestmentsID
