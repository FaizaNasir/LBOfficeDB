...
--  select * from dbo.[F_test6](29)            

CREATE FUNCTION [dbo].[F_PortfolioCal_ForeginCurrency]
(@vehicleID INT
)
     ...
     BEGIN
         INSERT INTO @result
                SELECT pv.STATUS, 
                       ...
                       (
                (
                    SELECT SUM(ISNULL(ForeignCurrencyAmount, 0))
                ...
                (
                    ...
                    (
                        ...
                        WHERE ShareholdingOperationID = pfp.ShareholdingOperationID
                                     ...
                                     THEN SUM(ISNULL(pfp.AmountDue, 0) * -1)
                                     ELSE SUM(ISNULL(pfp.AmountDue, 0))
                                 END
                ...
                (
                    SELECT ISNULL(SUM(ForeignCurrencyAmount), 0)
                ...
                --and      DATE <= GETDATE() ) as 'Last valuation'         
                       SUM([dbo].[F_Last_Valuation_ForeginCurrency](pv.VehicleID, p.PortfolioID, GETDATE())), 
                (
                    SELECT SUM(ForeignCurrencyAmount)
